...
const whiskySubcategoryImages: Record<string, string> = {
  ...
  "scotch-whisky": "/images/categories/scotch-whisky.png",
  "american-whisky": "/images/categories/american-whisky.jpg",
  "irish-whisky": "/images/categories/irish-whisky.jpg",
  "japanese-whisky": "/images/categories/japanese-whisky.jpg"
};

export default async function WhiskySubcategoryPage({
  params
}: {
  params: { slug: string };
}) {
  const currentPage = whiskySubcategoryPages.find((item) => item.slug === params.slug);

  if (!currentPage) {
    notFound();
  }

  const products = await getWhiskyProductsBySubcategory(currentPage.name, 24);
  const heroImage = whiskySubcategoryImages[currentPage.slug] ?? "/images/categories/whisky.jpg";
      ...
      <main className="mx-auto max-w-[1220px] px-4 pb-16 pt-8">
        <section className="rounded-[32px] border border-[#e9dfd1] bg-[linear-gradient(135deg,#fbf7f0_0%,#ffffff_55%,#f6f8ff_100%)] p-6 shadow-[0_18px_34px_rgba(0,0,0,0.06)] sm:p-8">
          ...
          <div className="mt-5 grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            ...
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#8b6a2b]">Whisky Collection</p>
        ...
        <section className="mt-12">
          <div className="flex flex-wrap items-end justify-between gap-4 border-b border-[#ece7de] pb-5">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#8b6a2b]">Subcategory products</p>
              <h2 className="mt-2 text-3xl font-extrabold text-[#171212]">สินค้าในหมวด {currentPage.name}</h2>
            </div>
            <p className="max-w-xl text-sm leading-7 text-[#5f5852]">
              สินค้าที่ถูกเพิ่มจากหลังบ้านและเลือกหมวดย่อยนี้ไว้ จะแสดงในหน้านี้โดยอัตโนมัติ
            </p>
          </div>
